...
function generate(registry, rawRequest, middleware) {
  return __async(this, null, function* () {
    ...
    for (const part of toolCalls) {
      ...
      if (!tool) {
        throw Error(`Tool ${(_b = part.toolRequest) == null ? void 0 : _b.name} not found`);
      }
      if (tool.__action.metadata.type === "prompt") {
        const newPreamble = yield tool((_c = part.toolRequest) == null ? void 0 : _c.input);
        toolResponses.push({
          toolResponse: {
        ...
        toolResponses.push({
          toolResponse: {
            name: part.toolRequest.name,
            ref: part.toolRequest.ref,
            output: yield tool((_d = part.toolRequest) == null ? void 0 : _d.input)
          }
        });
      }
    }
